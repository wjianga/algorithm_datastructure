...
#  Description: recursion practices; assignment 9

#  Student Name: Wenxuan Jiang

#  Student UT EID: wj3972
 
#  Partner's Name: Zixi Lei

#  Partner's UT EID: zl7732
...
def groupSum(start, nums, target):
    # base case:
    # return True if the sum of the group reach the target
    # return false all possible combinations of elements (groups) do not satisfy the target
    if start == len(nums):
        return (target == 0)
    # recursive case: return two branches: 1. the group considers the current element 2. do not cosider the current element
    else:
        # consider the current element
        return groupSum(start + 1, nums, target) \
            or groupSum(start + 1, nums, target - nums[start]) # do not consider the current element
...
def groupSum6(start, nums, target):
    # base case: the combinations of numbers equal to the target
    if target == 0 and start == len(nums):
        return True
    # base case: the combinations of numbers do not equal to the target
    elif start == len(nums):
        return False
    else:
        # recursive case: if the current number is 6, include it in the group
        if nums[start] == 6:
            return groupSum6(start + 1, nums, target - 6)
        else:
            # recursive case: if the current number is not 6, branch out to two cases
            return groupSum6(start + 1, nums, target) \
                or groupSum6(start + 1, nums, target - nums[start]) # do not consider the current element
...
def groupNoAdj(start, nums, target):
    ...
    if start >= len(nums):
        ...
        # if I choose to include the current element, I am going to skip the next element
        return groupNoAdj(start + 2, nums, target - nums[start]) \
            or groupNoAdj(start + 1, nums, target) # not include the current element
...
def groupSum5(start, nums, target):
    ...
    if start == len(nums):
        ...
        # if the current number is a multiple of 5, include it
        if nums[start] % 5 == 0:
            return groupSum5(start + 1, nums, target - nums[start])
        # if the current number is 1 and immediately follows a 5, skip the current number
        elif nums[start] == 1 and nums[start - 1] % 5 == 0:
            return groupSum5(start + 1, nums, target)
        else:
            # the current number is neither multiples of 5 not 1, normally branching out in 2
            return groupSum5(start + 1, nums, target) \
                or groupSum5(start + 1, nums, target - nums[start])
...
def groupSumClump(start, nums, target):
    ...
    if start == len(nums):
        ...
        # record the numbers of same number in a row from the current number
        adjNum = 0 
        for i in range(start, len(nums) - 1):
            if nums[start] == nums[i + 1]:
                ...
                break
        # two braches: either include all of the adjacent number or not include at all
        return groupSumClump(start + adjNum + 1, nums, target - (nums[start] * (adjNum + 1))) \
            or groupSumClump(start + adjNum + 1, nums, target)
...
# (No loops needed.)
def splitArray(nums):
    return splitArrayHelper(0, nums, [], [])

def splitArrayHelper(idx, nums, a, b):
    # base case
    if idx == len(nums):
        return sum(a) == sum(b)
    else:
        c = a[:]
        d = b[:]
        # include the current one in group A
        a.append(nums[idx])
        # include the current one in group B
        b.append(nums[idx])
        # 2 braches: include the current one in a or in b
        return splitArrayHelper(idx + 1, nums, a, d) or splitArrayHelper(idx + 1, nums, c, b)
...
def splitOdd10(nums):
    return splitOdd10Helper(0, nums, [], [])

def splitOdd10Helper(idx, nums, a, b):
    # base case:
    if idx == len(nums):
        return ((sum(a) % 10 == 0 and sum(b) % 2 == 1) or (sum(b) % 10 == 0 and sum(a) % 2 == 1))
    else:
        c = a[:]
        d = b[:]
        # include the current one in group A
        ...
        b.append(nums[idx])
        # 2 braches: include the current one in a or in b
        return splitOdd10Helper(idx + 1, nums, a, d) or splitOdd10Helper(idx + 1, nums, c, b)
...
def split53(nums):
    return split53Helper(0, nums, [], [])

def split53Helper(idx, nums, a, b):
    # base case
    if idx == len(nums):
        return sum(a) == sum(b)
    else:
        c = a[:]
        d = b[:]
        # if the current number is a multiple of 3 but not 5, inlcude it in group a
        if nums[idx] % 3 == 0 and nums[idx] % 5 != 0:
            a.append(nums[idx])
            return split53Helper(idx + 1, nums, a, d)
        # if the current number is a multiplt of 5, include it in group b
        elif nums[idx] % 5 == 0:
            b.append(nums[idx])
            return split53Helper(idx + 1, nums, c, b)
        # normally branch into a or b if neither a multiplt of 3 not 5
        else:
            a.append(nums[idx])
            b.append(nums[idx])
            return split53Helper(idx + 1, nums, c, b) or split53Helper(idx + 1, nums, a, d)
